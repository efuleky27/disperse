...
- Filament manifolds (optional): `<prefix>_sX_filament_manifolds_<TAG>_S###.vtu` (NDnet from `mse`: `<prefix>_sX_manifolds_<TAG>.NDnet`).
...
- Persistence tags (`sX`) precede the role/tag; smoothing (`S###`) is appended by converters. DisPerSE tools sometimes emit `.S###`, but outputs are normalized to `_S###` during the pipeline.
