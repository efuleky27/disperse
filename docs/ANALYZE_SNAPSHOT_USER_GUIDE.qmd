...
Below is a description of every argument and the overall workflow.
...
* `--dump-arcs CUID` — one or more `-dumpArcs` descriptors (e.g., `U`, `CUD`). Repeatable.
  - Note: DisPerSE concatenates multiple manifold tags into one output (e.g., `JE1a2a`). If you need separate files per tag, run `mse`/`analyze_snapshot.py` separately for each `--dump-manifolds`.
...
- Delaunay: `<prefix>_delaunay.S###.vtu` (or `_S000` for unsmoothed conversions).
- Manifolds: `<prefix>_sX_manifolds_<TAG>.S###.vtu` (NDnet: `<prefix>_sX_manifolds_<TAG>.NDnet`).
- Skeletons: `<prefix>_sX_arcs_<TAG>.S###.vtp` (NDskl: `<prefix>_sX_arcs_<TAG>.NDskl`).
- Only one dot is used (before the extension); persistence (`sX`) precedes the role/tag; smoothing (`S###`) is appended by converters.
...
6. **Validation**  
   Crop boxes are sanity-checked, projection parameters are normalized, and any reuse paths are expanded up front so errors surface early.

With these options, users can start from raw snapshots, run DisPerSE end to end, and visualize the cosmic web in ParaView.
