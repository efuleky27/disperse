...
output-root/
  crop_x<x0>-<x1>_y<y0>-<y1>_z<z0>-<z1>/
    ...
    slab_z<origin>/
      ...
      PNGs if batch_clip was called with --save-pngs (the driver passes it by default; PNG options such as `--png-dpi`, `--png-colormap`, `--png-log-range`, `--png-background`, `--png-transparent`, `--png-hide-orientation-axes`, `--png-lighting`, and `--composite-filaments-source` are forwarded)
```
Naming notes:
