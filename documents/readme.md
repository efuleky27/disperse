...
compare_simulations.py            → box plots, violin plots, and scatter plots across two simulation runs
...
Reads two simulations and renders box plots, violin plots, and scatter+marginal-density plots. Per-point `topology_points.csv` files are auto-discovered under each `--sim` folder for violin and scatter plots.
...
disperse/
  ...
  sessions/         AI conversation history
    ...
    export/         Rendered session exports (INDEX.md, MASTER_BY_DAY.qmd, MASTER_BY_DAY_flat.qmd, HTML)
```
...
`MASTER_BY_DAY.qmd` uses callout blocks with folded content. `MASTER_BY_DAY_flat.qmd` is an unfolded version of the same content, intended for PDF conversion. Both are regenerated by `export_sessions.py`; duplicate messages are deduplicated by UUID during export.
